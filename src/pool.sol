...
contract Pool is IPool {
    ...
    function creditTraderLoss(address user, string memory market, uint256 amount) external onlyTrade {
        ...
        if (lastPaid == 0) {
            ...
            amountToSendPool = Calculations.calculateAmountToSendPool(bufferBalance, lastPaid, bufferPayoutPeriod);
    ...
    function creditFee(address user, string memory market, uint256 fee, bool isLiquidation) external onlyTrade {
        ...
        uint256 poolFee = fee.calculateFeeAmount(store.poolFeeShare());
        uint256 treasuryFee = fee.calculateAmountMinusFee(poolFee);
